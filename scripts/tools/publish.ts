...
import type { BunLockFile } from "../types/bun-lock.js";
...
class ReleaseManager {
  ...
  static getNextVersion(latest: string) {
...
const main = async (): Promise<void> => {
  if (!await Bun.file("LATEST").exists()) throw new Error("Couldn't find LATEST version file");
  ...
  const [version, nextOrSha] = isCanary
    ...
    : ReleaseManager.getNextVersion(latest);
